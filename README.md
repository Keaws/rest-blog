...
# see the API below
$curl 'localhost:3003/api/posts?begin=0&length=10'

# get 10 posts after post with id 17. Same as above, the short version of the posts
# is returned
$curl 'localhost:3003/api/posts?after=17&length=10'

# get one post by ID
$curl 'localhost:3003/api/posts/38'

# add a post
$curl -H "Content-Type: application/json" -X POST -d '{"text":"hello there","title":"post"}' http://localhost:3003/api/posts

# delete one post by ID
$curl -X DELETE 'localhost:3003/api/posts/38'

# update post 38 with new data that is loaded from post.json file
$curl -X PUT -H "Content-Type: application/json" -d @post.json localhost:3003/api/post/38
